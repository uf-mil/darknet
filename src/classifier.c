...
float validate_classifier_single(char *datacfg, char *filename, char *weightfile, network *existing_net,
                                 int topk_custom);
...
void train_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear,
                      int dontuse_opencv, int dont_show, int mjpeg_port, int calc_topk, int show_imgs, char *chart_path)
{
  int i;
  ...
  {
    printf("\n Error: num of filters = %d in the last conv-layer in cfg-file doesn't match to classes = %d in "
           "data-file \n",
           l.outputs, classes);
    getchar();
  }
  ...
  if (!dontuse_opencv)
    img =
        draw_train_chart(windows_name, max_img_loss, net.max_batches, number_of_lines, img_size, dont_show, chart_path);
#endif  // OPENCV
  ...
  while (get_current_batch(net) < net.max_batches || net.max_batches == 0)
  {
    ...
    printf("%d, %.3f: %f, %f avg, %f rate, %lf seconds, %ld images, %f hours left\n", get_current_batch(net),
           (float)(*net.seen) / train_images_num, loss, avg_loss, get_current_rate(net), sec(clock() - time), *net.seen,
           avg_time);
#ifdef OPENCV
    if (net.contrastive)
    {
    ...
    if (!dontuse_opencv)
      draw_train_loss(windows_name, img, img_size, avg_loss, max_img_loss, i, net.max_batches, topk, draw_precision,
                      topk_buff, avg_contrastive_acc / 100, dont_show, mjpeg_port, avg_time);
#endif  // OPENCV
...
avg_loss, get_current_rate(net), sec(clock()-time), *net.seen);
if(*net.seen/N > epoch){
    epoch = *net.seen/N;
    char buff[256];
