  ...
  if (state.train)
  {
    ...
    if (l.batch_normalize == 2)
    {
      ...
      fast_v_cbn_gpu(l.output_gpu, l.mean_gpu, l.batch, l.out_c, l.out_h * l.out_w, minibatch_index,
                     max_minibatch_index, l.m_cbn_avg_gpu, l.v_cbn_avg_gpu, l.variance_gpu, alpha, l.rolling_mean_gpu,
                     l.rolling_variance_gpu, inverse_variance, .00001);

      normalize_scale_bias_gpu(l.output_gpu, l.mean_gpu, l.variance_gpu, l.scales_gpu, l.biases_gpu, l.batch, l.out_c,
                               l.out_h * l.out_w, inverse_variance, .00001f);
    ...
    else
    {
      ...
      float one = 1;
      float zero = 0;
      cudnnBatchNormalizationForwardTraining(cudnn_handle(), CUDNN_BATCHNORM_SPATIAL, &one, &zero, l.normDstTensorDesc,
                                             l.x_gpu,  // input
                                             l.normDstTensorDesc,
                                             l.output_gpu,  // output
...
void backward_batchnorm_layer_gpu(layer l, network_state state)
{
  ...
  float one = 1;
  float zero = 0;
  cudnnBatchNormalizationBackward(cudnn_handle(), CUDNN_BATCHNORM_SPATIAL, &one, &zero, &one, &one, l.normDstTensorDesc,
                                  l.x_gpu,  // input
                                  l.normDstTensorDesc,
                                  l.delta_gpu,  // input
                                  l.normDstTensorDesc,
