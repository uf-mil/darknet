                   ...
                   int cur_b, float *dst)
{
  ...
  {
    const int src_index = cur_b * (src_c * src_h * src_w) + cur_n * (embedding_size * src_h * src_w) +
                          i * src_h * src_w + cur_h * (src_w) + cur_w;
                           ...
                           contrastive_params *contrast_p, int contrast_p_size)
{
  ...
  for (k = 0; k < contrast_p_size; ++k)
  {
    ...
    if (cp.i != i && cp.j == j)
    {
      ...
      ////const float sim_den = find_sim(k, l, contrast_p, contrast_p_size); // cosine_similarity(z[k], z[l],
      /// feature_size);
...
void grad_contrastive_loss_positive_f(size_t i, int *labels, size_t num_of_samples, float **z,
                                      ...
                                      contrastive_params *contrast_p, int contrast_p_size)
{
  ...
  for (j = 0; j < num_of_samples; ++j)
  {
    ...
    if (i != j && labels[i] == labels[j] && labels[i] >= 0)
    {
      ...
      // const float sim = find_sim(i, j, contrast_p, contrast_p_size); //cos_sim[i*num_of_samples + j];        //
      // cosine_similarity(z[i], z[j], feature_size);
      // const float P = find_P_constrastive(i, j, contrast_p, contrast_p_size); //p_constrastive[i*num_of_samples + j];
      // // P_constrastive(i, j, labels, num_of_samples, z, feature_size, temperature, cos_sim);
...
void grad_contrastive_loss_negative_f(size_t i, int *labels, size_t num_of_samples, float **z,
                                      unsigned int feature_size, float temperature, float *delta, int wh,
                                      contrastive_params *contrast_p, int contrast_p_size)
{
  const float vec_len = math_vector_length(z[i], feature_size);
    ...
    if (i != j && labels[i] == labels[j] && labels[i] >= 0)
    {
      ...
      for (k = 0; k < num_of_samples; ++k)
      {
        ...
        if (k != i && k != j && labels[k] >= 0)
        {
          ...
          // const float sim = find_sim(i, k, contrast_p, contrast_p_size); //cos_sim[i*num_of_samples + k];        //
          // cosine_similarity(z[i], z[k], feature_size);
          // const float P = find_P_constrastive(i, k, contrast_p, contrast_p_size); //p_constrastive[i*num_of_samples +
...
// z[feature_size][num_of_samples] - array of arrays with contrastive features (output of conv-layer, f.e. 128 floats
// for each sample)
                                    ...
                                    float temperature, float *cos_sim, float *p_constrastive, float *delta, int wh)
{
  ...
  for (j = 0; j < num_of_samples; ++j)
  {
    ...
    if (i != j && labels[i] == labels[j])
    {
      ...
      const float P = p_constrastive[i * num_of_samples + j];  // P_constrastive(i, j, labels, num_of_samples, z,
                                                               // feature_size, temperature, cos_sim);
      // const float custom_pos_mult = 1 - sim;
    ...
    if (i != j && labels[i] == labels[j])
    {
      ...
      for (k = 0; k < num_of_samples; ++k)
      {
        ...
        if (k != i && k != j && labels[k] >= 0)
        {
          ...
          const float P = p_constrastive[i * num_of_samples + k];  // P_constrastive(i, k, labels, num_of_samples, z,
                                                                   // feature_size, temperature, cos_sim);
          // const float custom_pos_mult = 1 + sim;
