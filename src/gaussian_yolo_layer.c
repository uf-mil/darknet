...
void forward_gaussian_yolo_layer(const layer l, network_state state)
{
  ...
  for (b = 0; b < l.batch; ++b)
  {
    for (j = 0; j < l.h; ++j)
    {
      for (i = 0; i < l.w; ++i)
      {
        for (n = 0; n < l.n; ++n)
        {
          ...
          box pred = get_gaussian_yolo_box(l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h, state.net.w,
                                           state.net.h, l.w * l.h, l.yolo_point);
          ...
          for (t = 0; t < l.max_boxes; ++t)
          {
            ...
            int class_id_match =
                compare_gaussian_yolo_class(l.output, l.classes, class_index, l.w * l.h, objectness, class_id, 0.25f);
          ...
          if (best_match_iou > l.ignore_thresh)
          {
            const float iou_multiplier =
                best_match_iou * best_match_iou;  // (best_match_iou - l.ignore_thresh) / (1.0 - l.ignore_thresh);
          ...
          if (best_iou > l.truth_thresh)
          {
            ...
            delta_gaussian_yolo_box(truth, l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h, state.net.w,
                                    state.net.h, l.delta, (2 - truth.w * truth.h), l.w * l.h,
                                    l.iou_normalizer * class_multiplier, l.iou_loss, l.uc_normalizer, 1, l.yolo_point,
                                    l.max_delta);
          }
        }
      }
    ...
    for (t = 0; t < l.max_boxes; ++t)
    {
      ...
      if (mask_n >= 0)
      {
        ...
        const float class_multiplier = (l.classes_multipliers) ? l.classes_multipliers[class_id] : 1.0f;
        float iou = delta_gaussian_yolo_box(truth, l.output, l.biases, best_n, box_index, i, j, l.w, l.h, state.net.w,
                                            state.net.h, l.delta, (2 - truth.w * truth.h), l.w * l.h,
      ...
      for (n = 0; n < l.total; ++n)
      {
        ...
        if (mask_n >= 0 && n != best_n && l.iou_thresh < 1.0f)
        {
          ...
          if (iou > l.iou_thresh)
          {
            ...
            int box_index = entry_gaussian_index(l, b, mask_n * l.w * l.h + j * l.w + i, 0);
            const float class_multiplier = (l.classes_multipliers) ? l.classes_multipliers[class_id] : 1.0f;
            float iou = delta_gaussian_yolo_box(truth, l.output, l.biases, n, box_index, i, j, l.w, l.h, state.net.w,
                                                state.net.h, l.delta, (2 - truth.w * truth.h), l.w * l.h,
                                                l.iou_normalizer * class_multiplier, l.iou_loss, l.uc_normalizer, 1,
                                                l.yolo_point, l.max_delta);
  ...
  *(l.cost) = pow(mag_array(l.delta, l.outputs * l.batch), 2);
  ...
  fprintf(stderr, "Region %d Avg IOU: %f, Class: %f, Obj: %f, No Obj: %f, .5R: %f, .75R: %f,  count: %d, class_loss = "
                  "%.2f, iou_loss = %.2f, uc_loss = %.2f, total_loss = %.2f \n",
          state.index, avg_iou / count, avg_cat / class_count, avg_obj / count,
          avg_anyobj / (l.w * l.h * l.n * l.batch), recall / count, recall75 / count, count, class_loss, iou_loss,
          uc_loss, loss);
}
  ...
  for (i = 0; i < l.w * l.h; ++i)
  {
    ...
    for (n = 0; n < l.n; ++n)
    {
      ...
      if (objectness > thresh)
      {
        ...
        dets[count].bbox = get_gaussian_yolo_box(predictions, l.biases, l.mask[n], box_index, col, row, l.w, l.h, netw,
                                                 neth, l.w * l.h, l.yolo_point);
