...
void resize_yolo_layer(layer *l, int w, int h)
{
  ...
  if (l->embedding_output)
    l->embedding_output =
        (float *)xrealloc(l->output, l->batch * l->embedding_size * l->n * l->h * l->w * sizeof(float));
  if (l->labels)
                    ...
                    float max_delta, int *rewritten_bbox)
{
  ...
  if (iou_loss == MSE)  // old loss
  {
    ...
    // printf(" x = %f, y = %f, w = %f, h = %f \n", x[index + 0 * stride], x[index + 1 * stride], x[index + 2 * stride],
    // x[index + 3 * stride]);
  ...
  *(l.cost) = 0;
  for (b = 0; b < l.batch; ++b)
  {
    for (j = 0; j < l.h; ++j)
    {
      for (i = 0; i < l.w; ++i)
      {
        for (n = 0; n < l.n; ++n)
        {
          ...
          box pred = get_yolo_box(l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h, state.net.w, state.net.h,
                                  l.w * l.h);
          ...
          for (t = 0; t < l.max_boxes; ++t)
          {
            ...
            {
              printf("\n Warning: in txt-labels class_id=%d >= classes=%d in cfg-file. In txt-labels class_id should "
                     "be [from 0 to %d] \n",
                     class_id, l.classes, l.classes - 1);
              printf("\n truth.x = %f, truth.y = %f, truth.w = %f, truth.h = %f, class_id = %d \n", truth.x, truth.y,
                     truth.w, truth.h, class_id);
              if (check_mistakes)
                getchar();
              continue;  // if label contains class_id more than number of classes in the cfg-file and class_id check
                         // garbage value
            }
            ...
            int class_id_match =
                compare_yolo_class(l.output, l.classes, class_index, l.w * l.h, objectness, class_id, 0.25f);
          ...
          if (best_iou > l.truth_thresh)
          {
            ...
            if (l.objectness_smooth)
              l.delta[class_index + stride * class_id] =
            ...
            delta_yolo_box(truth, l.output, l.biases, l.mask[n], box_index, i, j, l.w, l.h, state.net.w, state.net.h,
                           l.delta, (2 - truth.w * truth.h), l.w * l.h, l.iou_normalizer * class_multiplier, l.iou_loss,
                           1, l.max_delta, state.net.rewritten_bbox);
    ...
    for (t = 0; t < l.max_boxes; ++t)
    {
      ...
      int class_id = state.truth[t * l.truth_size + b * l.truths + 4];
      if (class_id >= l.classes || class_id < 0)
        continue;  // if label contains class_id more than number of classes in the cfg-file and class_id check garbage
                   // value
      ...
      if (mask_n >= 0)
      {
        ...
        const float class_multiplier = (l.classes_multipliers) ? l.classes_multipliers[class_id] : 1.0f;
        ious all_ious =
            delta_yolo_box(truth, l.output, l.biases, best_n, box_index, i, j, l.w, l.h, state.net.w, state.net.h,
                           l.delta, (2 - truth.w * truth.h), l.w * l.h, l.iou_normalizer * class_multiplier, l.iou_loss,
                           1, l.max_delta, state.net.rewritten_bbox);
        (*state.net.total_bbox)++;
        ...
        // truth_in_index, truth_out_index);
        ...
        // truth.y, truth.w, truth.h);
        // printf(" mask_n = %d, l.output[obj_index] = %f, l.output[class_index + class_id] = %f \n\n", mask_n,
        // l.output[obj_index], l.output[class_index + class_id]);
      ...
      for (n = 0; n < l.total; ++n)
      {
        ...
        if (mask_n >= 0 && n != best_n && l.iou_thresh < 1.0f)
        {
          ...
          if (iou > l.iou_thresh)
          {
            ...
            const float class_multiplier = (l.classes_multipliers) ? l.classes_multipliers[class_id] : 1.0f;
            ious all_ious =
                delta_yolo_box(truth, l.output, l.biases, n, box_index, i, j, l.w, l.h, state.net.w, state.net.h,
                               l.delta, (2 - truth.w * truth.h), l.w * l.h, l.iou_normalizer * class_multiplier,
  ...
  // avg_iou / count, avg_cat / class_count, avg_obj / count, avg_anyobj / (l.w*l.h*l.n*l.batch), recall / count,
  ...
  fprintf(stderr, "v3 (%s loss, Normalizer: (iou: %.2f, cls: %.2f) Region %d Avg (IOU: %f, GIOU: %f), Class: %f, Obj: "
                  "%f, No Obj: %f, .5R: %f, .75R: %f, count: %d, class_loss = %f, iou_loss = %f, total_loss = %f \n",
          ...
          avg_anyobj / (l.w * l.h * l.n * l.batch), recall / count, recall75 / count, count, classification_loss,
          iou_loss, loss);
}
  ...
  for (i = 0; i < l.w * l.h; ++i)
  {
    ...
    for (n = 0; n < l.n; ++n)
    {
      ...
      if (objectness > thresh)
      {
        ...
        if (l.embedding_output)
        {
          get_embedding(l.embedding_output, l.w, l.h, l.n * l.embedding_size, l.embedding_size, col, row, n, 0,
  ...
  for (i = 0; i < l.w * l.h; ++i)
  {
    ...
    for (n = 0; n < l.n; ++n)
    {
      ...
      if (objectness > thresh)
      {
        ...
        if (l.embedding_output)
        {
          get_embedding(l.embedding_output, l.w, l.h, l.n * l.embedding_size, l.embedding_size, col, row, n, batch,
                        dets[count].embeddings);
        }
