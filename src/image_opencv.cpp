...
#define OPENCV_VERSION                                                                                                 \
  CVAUX_STR(CV_VERSION_MAJOR) "" CVAUX_STR(CV_VERSION_MINOR) "" CVAUX_STR(CV_VERSION_REVISION) OCV_D
...
// Draw Detection
// ====================================================================
extern "C" void draw_detections_cv_v3(mat_cv *mat, detection *dets, int num, float thresh, char **names,
  ...
  try
  {
    ...
    for (i = 0; i < num; ++i)
    {
      ...
      if (class_id >= 0)
      {
        ...
        //    if(copy_img == NULL) copy_img = cvCreateImage(cvSize(show_img->width, show_img->height), show_img->depth,
        //    show_img->nChannels);
        ...
        if (ext_output)
          printf("\t(left_x: %4.0f   top_y: %4.0f   width: %4.0f   height: %4.0f)\n", (float)left, (float)top,
                 b.w * show_img->cols, b.h * show_img->rows);
        else
          printf("\n");
        ...
        cv::putText(*show_img, labelstr, pt_text, cv::FONT_HERSHEY_COMPLEX_SMALL, font_size, black_color, 2 * font_size,
                    CV_AA);
                                    ...
                                    int img_size, int dont_show, char *chart_path)
{
  ...
  try
  {
    ...
    else
    {
      ...
      cv::putText(img, "Iteration number", cv::Point(draw_size / 2, img_size - 10), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7,
                  CV_RGB(0, 0, 0), 1, CV_AA);
      char max_batches_buff[100];
      sprintf(max_batches_buff, "in cfg max_batches=%d", max_batches);
      cv::putText(img, max_batches_buff, cv::Point(draw_size - 195, img_size - 10), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7,
                  CV_RGB(0, 0, 0), 1, CV_AA);
      cv::putText(img, "Press 's' to save : chart.png", cv::Point(5, img_size - 10), cv::FONT_HERSHEY_COMPLEX_SMALL,
...
extern "C" void draw_train_loss(char *windows_name, mat_cv *img_src, int img_size, float avg_loss, float max_img_loss,
                                int current_batch, int max_batches, float precision, int draw_precision,
                                char *accuracy_name, float contr_acc, int dont_show, int mjpeg_port,
                                double time_remaining)
{
  try
  {
    ...
    if (contr_acc >= 0)
    {
      ...
      if (current_batch > 0)
      {
        cv::line(img, cv::Point(img_offset + draw_size * (float)(current_batch - 1) / max_batches,
    ...
    if (draw_precision)
    {
      ...
      if (iteration_old == 0)
        cv::putText(img, accuracy_name, cv::Point(10, 12), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(255, 0, 0), 1,
                    CV_AA);

      if (iteration_old != 0)
      {
        cv::line(img, cv::Point(img_offset + draw_size * (float)iteration_old / max_batches,
      ...
      cv::putText(img, char_buff, cv::Point(10, 28), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(255, 255, 255), 5,
                  CV_AA);
    ...
    if (k == 's' || current_batch == (max_batches - 1) || (current_batch / 100 > old_batch / 100))
    {
      ...
      cv::putText(img, "- Saved", cv::Point(260, img_size - 10), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(255, 0, 0),
                  1, CV_AA);
    }
    else
...
extern "C" image image_data_augmentation(mat_cv *mat, int w, int h, int pleft, int ptop, int swidth, int sheight,
                                         int flip, float dhue, float dsat, float dexp, int gaussian_noise, int blur,
                                         int num_boxes, int truth_size, float *truth)
{
  image out;
  try
  {
                               ...
                               float *lr_set, int *boxonly, int classes, char **names)
{
  ...
  while (!selected)
  {
    ...
    if (names)
    {
      ...
      cv::putText(frame_clone, obj_name, cv::Point2i(10, 40), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(20, 120, 60),
                  2);
      cv::putText(frame_clone, obj_name, cv::Point2i(10, 40), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(50, 200, 100),
                  1);
    }

    if (draw_select)
    {
      cv::Rect selected_rect(cv::Point2i((int)min(x_start, x_end), (int)min(y_start, y_end)), cv::Size(x_size, y_size));
...
// Show Anchors
// ====================================================================
extern "C" void show_acnhors(int number_of_boxes, int num_of_clusters, float *rel_width_height_array,
...
void show_opencv_info()
{
  std::cerr << " OpenCV version: " << CV_VERSION_MAJOR << "." << CV_VERSION_MINOR << "."
            << CVAUX_STR(CV_VERSION_REVISION) OCV_D << std::endl;
}
