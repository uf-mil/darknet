    ...
    for (j = 0; j < classes; ++j)
    {
      if (probs[i][j])
        fprintf(fp, "{\"image_id\":%d, \"category_id\":%d, \"bbox\":[%f, %f, %f, %f], \"score\":%f},\n", image_id,
                coco_ids[j], bx, by, bw, bh, probs[i][j]);
    }
  }
}
...
void run_coco(int argc, char **argv)
{
  ...
  if (0 == strcmp(argv[2], "test"))
    ...
    demo(cfg, weights, thresh, hier_thresh, cam_index, filename, coco_classes, 80, 1, frame_skip, prefix, out_filename,
         mjpeg_port, 0, json_port, dont_show, ext_output, 0, 0, 0, 0, 0);
}
