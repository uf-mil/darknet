...
layer make_conv_lstm_layer(int batch, int h, int w, int c, int output_filters, int groups, int steps, int size,
                           int stride, int dilation, int pad, ACTIVATION activation, int batch_normalize, int peephole,
                           int xnor, int bottleneck, int train)
{
  ...
  *(l.uf) = make_convolutional_layer(batch, steps, h, w, c, output_filters, groups, size, stride, stride, dilation, pad,
                                     activation, batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
  l.uf->batch = batch;
  if (l.workspace_size < l.uf->workspace_size)
    l.workspace_size = l.uf->workspace_size;

  l.ui = (layer *)xcalloc(1, sizeof(layer));
  *(l.ui) = make_convolutional_layer(batch, steps, h, w, c, output_filters, groups, size, stride, stride, dilation, pad,
                                     activation, batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
  l.ui->batch = batch;
  ...
  *(l.ug) = make_convolutional_layer(batch, steps, h, w, c, output_filters, groups, size, stride, stride, dilation, pad,
                                     activation, batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
  l.ug->batch = batch;
  ...
  *(l.uo) = make_convolutional_layer(batch, steps, h, w, c, output_filters, groups, size, stride, stride, dilation, pad,
                                     activation, batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
  l.uo->batch = batch;
  ...
  if (l.bottleneck)
  {
    ...
    *(l.wf) =
        make_convolutional_layer(batch, steps, h, w, output_filters * 2, output_filters, groups, size, stride, stride,
                                 dilation, pad, activation, batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
    l.wf->batch = batch;
  ...
  else
  {
    ...
    l.wf = (layer *)xcalloc(1, sizeof(layer));
    *(l.wf) =
        make_convolutional_layer(batch, steps, h, w, output_filters, output_filters, groups, size, stride, stride,
                                 dilation, pad, activation, batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
    l.wf->batch = batch;
    if (l.workspace_size < l.wf->workspace_size)
      l.workspace_size = l.wf->workspace_size;
...
*/
  ...
  l.bflops = l.uf->bflops + l.ui->bflops + l.ug->bflops + l.uo->bflops + l.wf->bflops + l.wi->bflops + l.wg->bflops +
             l.wo->bflops + l.vf->bflops + l.vi->bflops + l.vo->bflops;
...
void backward_conv_lstm_layer_gpu(layer l, network_state state)
{
  ...
  for (i = l.steps - 1; i >= 0; --i)
  {
    ...
    // delta for c,f,i,g(w,u,v): delta_c = temp = (delta * o * grad_tanh(tanh(c)) + delta_c(???)) * f    //
    // (grad_linear(c)==1)
