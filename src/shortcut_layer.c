...
layer make_shortcut_layer(int batch, int n, int *input_layers, int *input_sizes, int w, int h, int c,
                          ...
                          WEIGHTS_NORMALIZATION_T weights_normalization, ACTIVATION activation, int train)
{
...
#pragma omp parallel for
  ...
  // shortcut_cpu(l.batch, from_w, from_h, from_c, state.net.layers[l.index].output, l.out_w, l.out_h, l.out_c,
  // l.output);
...
void backward_shortcut_layer(const layer l, network_state state)
{
  ...
  backward_shortcut_multilayer_cpu(l.outputs * l.batch, l.outputs, l.batch, l.n, l.input_sizes, l.layers_delta,
                                   state.delta, l.delta, l.weights, l.weight_updates, l.nweights, state.input,
...
void forward_shortcut_layer_gpu(const layer l, network_state state)
{
  ...
  // shortcut_gpu(l.batch, l.w, l.h, l.c, state.net.layers[l.index].output_gpu, l.out_w, l.out_h, l.out_c,
  // l.output_gpu);

  // input_shortcut_gpu(state.input, l.batch, l.w, l.h, l.c, state.net.layers[l.index].output_gpu, l.out_w, l.out_h,
  // l.out_c, l.output_gpu);

  //-----------
  // if (l.outputs == l.input_sizes[0])
  // if(l.n == 1 && l.nweights == 0)
  //{
  //    input_shortcut_gpu(state.input, l.batch, state.net.layers[l.index].w, state.net.layers[l.index].h,
  //    state.net.layers[l.index].c,
  ...
  // else
  {
    shortcut_multilayer_gpu(l.outputs, l.batch, l.n, l.input_sizes_gpu, l.layers_output_gpu, l.output_gpu, state.input,
                            l.weights_gpu, l.nweights, l.weights_normalization);
  }
...
void backward_shortcut_layer_gpu(const layer l, network_state state)
{
  ...
  backward_shortcut_multilayer_gpu(l.outputs, l.batch, l.n, l.input_sizes_gpu, l.layers_delta_gpu, state.delta,
                                   l.delta_gpu, l.weights_gpu, l.weight_updates_gpu, l.nweights, state.input,
