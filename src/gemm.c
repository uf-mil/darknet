...
uint32_t reverse_32_bit(uint32_t a)
{
  ...
  return (reverse_8_bit(a >> 24) << 0) | (reverse_8_bit(a >> 16) << 8) | (reverse_8_bit(a >> 8) << 16) |
...
{
  return ((a & 0x1) << 7) | ((a & 0x2) << 5) | ((a & 0x4) << 3) | ((a & 0x8) << 1) | ((a & 0x10) >> 1) |
         ((a & 0x20) >> 3) | ((a & 0x40) >> 5) | ((a & 0x80) >> 7);
}
...
// transpose by 32-bit
void transpose_bin(uint32_t *A, uint32_t *B, const int n, const int m, const int lda, const int ldb,
...
void gemm_nn_bin_32bit_packed(int M, int N, int K, float ALPHA, uint32_t *A, int lda, uint32_t *B, int ldb, float *C,
                              int ldc, float *mean_arr)
{
  int i;
#pragma omp parallel for
  for (i = 0; i < M; ++i)
  {  // l.n
    ...
    for (s = 0; s < K; ++s)  // l.size*l.size*l.c/32  or (l.size*l.size*l.c)
    {
      ...
      for (j = 0; j < N - 8; j += 8)
      {
        ...
        __m256 count =
            _mm256_setr_ps(popcnt_32(_mm256_extract_epi32(xnor256, 0)), popcnt_32(_mm256_extract_epi32(xnor256, 1)),
                           popcnt_32(_mm256_extract_epi32(xnor256, 2)), popcnt_32(_mm256_extract_epi32(xnor256, 3)),
                           popcnt_32(_mm256_extract_epi32(xnor256, 4)), popcnt_32(_mm256_extract_epi32(xnor256, 5)),
                           popcnt_32(_mm256_extract_epi32(xnor256, 6)), popcnt_32(_mm256_extract_epi32(xnor256, 7)));
...
void convolution_2d_old(int w, int h, int ksize, int n, int c, int pad, int stride, float *weights, float *input,
                        float *output)
{
...
void convolution_2d(int w, int h, int ksize, int n, int c, int pad, int stride, float *weights, float *input,
                    float *output, float *mean)
{
  ...
  __m256i all256_sing1 =
      _mm256_set_epi32(0x80000000, 0x80000000, 0x80000000, 0x80000000, 0x80000000, 0x80000000, 0x80000000, 0x80000000);
        ...
        for (chan = 0; chan < c; ++chan)
        {
          ...
          for (f_y = 0; f_y < ksize; ++f_y)
          {
            ...
            for (f_x = 0; f_x < ksize; ++f_x)
            {
              ...
              // in.m256_f32[1], in.m256_f32[2], in.m256_f32[3], in.m256_f32[4], in.m256_f32[5], in.m256_f32[6],
              // in.m256_f32[7]);
...
static inline int popcnt256_custom(__m256i n)
{
  ...
  // val.m256i_i64[2] +
  // val.m256i_i64[3];
  return _mm256_extract_epi64(val, 0) + _mm256_extract_epi64(val, 1) + _mm256_extract_epi64(val, 2) +
...
static inline void xnor_avx2_popcnt(__m256i a_bit256, __m256i b_bit256, __m256i *count_sum)
{
  ...
  c_bit256 = _mm256_andnot_si256(
      xor256, c_bit256);  // can be optimized - we can do other NOT for wegihts once and do not do this NOT
...
{
  return _mm256_extract_epi64(count_sum, 0) + _mm256_extract_epi64(count_sum, 1) + _mm256_extract_epi64(count_sum, 2) +
         _mm256_extract_epi64(count_sum, 3);
}

// 5x times faster than gemm()-float32
// further optimizations: do mean-mult only for the last layer
void gemm_nn_custom_bin_mean_transposed(int M, int N, int K, float ALPHA_UNUSED, unsigned char *A, int lda,
                                        unsigned char *B, int ldb, float *C, int ldc, float *mean_arr)
{
  int i;
...
void im2col_cpu_custom_transpose(float *data_im, int channels, int height, int width, int ksize, int stride, int pad,
                                 float *data_col, int ldb_align)
{
...
// From Berkeley Vision's Caffe!
// https://github.com/BVLC/caffe/blob/master/LICENSE
void im2col_cpu_custom(float *data_im, int channels, int height, int width, int ksize, int stride, int pad,
                       float *data_col)
{
  int c;
  const int height_col = (height + 2 * pad - ksize) / stride + 1;
...
// From Berkeley Vision's Caffe!
// https://github.com/BVLC/caffe/blob/master/LICENSE
void im2col_cpu_custom_align(float *data_im, int channels, int height, int width, int ksize, int stride, int pad,
                             float *data_col, int bit_align)
{
  int c;
  const int height_col = (height + 2 * pad - ksize) / stride + 1;
  const int width_col = (width + 2 * pad - ksize) / stride + 1;
...
#pragma omp parallel for
  ...
  {
    printf("\n Error: is no non-optimized version \n");
    // im2col_cpu(data_im, channels, height, width, ksize, stride, pad, data_col); // must be aligned for transpose
    // after float_to_bin
...
// From Berkeley Vision's Caffe!
// https://github.com/BVLC/caffe/blob/master/LICENSE
void im2col_cpu_custom_bin(float *data_im, int channels, int height, int width, int ksize, int stride, int pad,
                           float *data_col, int bit_align)
{
  int c;
  const int height_col = (height + 2 * pad - ksize) / stride + 1;
  ...
  else if (a == LEAKY)
  {
    if (is_fma_avx2())
    {
      ...
      for (i = 0; i < n - 8; i += 8)
      {
        ...
        __m256i sign256 =
            _mm256_and_si256(_mm256_castps_si256(src256), all256_sing1);  // check sign in 8 x 32-bit floats
...
void transpose_block_SSE4x4(float *A, float *B, const int n, const int m, const int lda, const int ldb,
                            const int block_size)
{
  int i;
#pragma omp parallel for
...
void forward_maxpool_layer_avx(float *src, float *dst, int *indexes, int size, int w, int h, int out_w, int out_h,
...
void gemm_nn_bin_transposed_32bit_packed(int M, int N, int K, float ALPHA, uint32_t *A, int lda, uint32_t *B, int ldb,
                                         float *C, int ldc, float *mean_arr)
{
  int i;
#pragma omp parallel for
...
void convolution_repacked(uint32_t *packed_input, uint32_t *packed_weights, float *output, int w, int h, int c, int n,
                          int size, int pad, int new_lda, float *mean_arr)
{
  int fil;
// filter index
#pragma omp parallel for
        ...
        for (x = 0; x < w; ++x)
        {
          ...
          for (f_y = 0; f_y < size; ++f_y)
          {
            ...
            for (f_x = 0; f_x < size; ++f_x)
            {
              ...
              // float weight = l.weights[fil*l.c*l.size*l.size + chan*l.size*l.size + (f_y*l.size + f_x) * 32 +
              // c_pack];
              // sum += input * weight;
              ...
              // printf("\n mul = %f, bit1 = %d, bit2 = %d, count = %d, mean = %f, result = %f  ", input*weight, bit1,
              // bit2, count, mean_val, result);
              // sum += result;

              uint32_t input = ((uint32_t *)packed_input)[chan * w * h + input_y * w + input_x];
              // uint32_t weight = ((uint32_t *)l.align_bit_weights)[fil*l.c*l.size*l.size/32 + chan*l.size*l.size +
              // f_y*l.size + f_x];
              uint32_t weight =
                  ((uint32_t *)packed_weights)[fil * new_lda / 32 + chan * size * size + f_y * size + f_x];
...
void gemm_ongpu(int TA, int TB, int M, int N, int K, float ALPHA, float *A_gpu, int lda, float *B_gpu, int ldb,
                float BETA, float *C_gpu, int ldc)
{
  ...
  cudaError_t status =
      (cudaError_t)cublasSgemm(handle, (TB ? CUBLAS_OP_T : CUBLAS_OP_N), (TA ? CUBLAS_OP_T : CUBLAS_OP_N), N, M, K,
