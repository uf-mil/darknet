...
void correct_boxes(box_label *boxes, int n, float dx, float dy, float sx, float sy, int flip)
{
  ...
  for (i = 0; i < n; ++i)
  {
    ...
    if ((boxes[i].x + boxes[i].w / 2) < 0 || (boxes[i].y + boxes[i].h / 2) < 0 || (boxes[i].x - boxes[i].w / 2) > 1 ||
        (boxes[i].y - boxes[i].h / 2) > 1)
    {
      boxes[i].x = 999999;
      boxes[i].y = 999999;
      boxes[i].w = 999999;
                         ...
                         float dy, float sx, float sy, int net_w, int net_h)
{
  ...
  for (i = 0; i < count; ++i)
  {
    ...
    // float val = track_id;
    // printf(" i = %d, sub = %d, truth_size = %d, track_id = %d, %f, %f\n", i, sub, truth_size, track_id, truth[(i -
    // sub)*truth_size + 5], val);
                        ...
                        int net_w, int net_h, int mosaic_bound)
{
  ...
  for (t = count_new_truth; t < boxes; ++t)
  {
    ...
    if (left >= 0 && right <= w && top >= 0 && bot <= h && wb > 0 && wb < 1 && hb > 0 && hb < 1 && xb > 0 && xb < 1 &&
        yb > 0 && yb < 1 && wb > lowest_w && hb > lowest_h)
    {
      new_truth_ptr[0] = xb;
      new_truth_ptr[1] = yb;
      new_truth_ptr[2] = wb;
...
data load_data_detection(int n, char **paths, int m, int w, int h, int c, int boxes, int truth_size, int classes,
                         int use_flip, int use_gaussian_noise, int use_blur, int use_mixup, float jitter, float resize,
                         float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed,
                         int letter_box, int mosaic_bound, int contrastive, int contrastive_jit_flip, int show_imgs)
{
...
void *load_thread(void *ptr)
{
  ...
  else if (a.type == DETECTION_DATA)
  {
    *a.d = load_data_detection(a.n, a.paths, a.m, a.w, a.h, a.c, a.num_boxes, a.truth_size, a.classes, a.flip,
                               a.gaussian_noise, a.blur, a.mixup, a.jitter, a.resize, a.hue, a.saturation, a.exposure,
                               a.mini_batch, a.track, a.augment_speed, a.letter_box, a.mosaic_bound, a.contrastive,
                               a.contrastive_jit_flip, a.show_imgs);
  }
    ...
    for (i = 0; i < d2.X.rows; ++i)
    {
      ...
      else if (mixup == 3)
      {
        ...
        for (j = 0; j < d.y.cols; ++j)
        {
          ...
          d.y.vals[i][j] = d.y.vals[i][j] * s1 / max_s + d2.y.vals[i][j] * s2 / max_s + d3.y.vals[i][j] * s3 / max_s +
