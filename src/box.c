...
dxrep dx_box_iou(box pred, box truth, IOU_LOSS iou_loss)
{
  ...
  printf("pred: x,y,w,h: (%f, %f, %f, %f) -> t,b,l,r: (%f, %f, %f, %f)\n", pred.x, pred.y, pred.w, pred.h,
         pred_tblr.top, pred_tblr.bot, pred_tblr.left, pred_tblr.right);
  ...
  ddx.dt = p_dx;  // We follow the original code released from GDarknet. So in yolo_layer.c, dt, db, dl, dr are already
                  // dx, dy, dw, dh.
  ddx.db = p_dy;
  ddx.dl = p_dw;
  ddx.dr = p_dh;
