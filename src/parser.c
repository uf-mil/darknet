...
convolutional_layer parse_convolutional(list *options, size_params params)
{
  ...
  if (deform && size == 1)
  {
    printf(" Error: params (sway=1, rotate=1 or stretch=1) should be used only with size >=3 in the [convolutional] "
           "layer \n");
    exit(0);
  }

  convolutional_layer layer =
      make_convolutional_layer(batch, 1, h, w, c, n, groups, size, stride_x, stride_y, dilation, padding, activation,
                               batch_normalize, binary, xnor, params.net.adam, use_bin_output, params.index,
...
layer parse_conv_lstm(list *options, size_params params)
{
  ...
  layer l = make_conv_lstm_layer(params.batch, params.h, params.w, params.c, output_filters, groups, params.time_steps,
                                 size, stride, dilation, padding, activation, batch_normalize, peephole, xnor,
                                 bottleneck, params.train);
...
contrastive_layer parse_contrastive(list *options, size_params params)
{
  ...
  contrastive_layer layer =
      make_contrastive_layer(params.batch, params.w, params.h, params.c, classes, params.inputs, yolo_layer);
  layer.temperature = option_find_float_quiet(options, "temperature", 1);
...
layer parse_yolo(list *options, size_params params)
{
  ...
  fprintf(stderr, "[yolo] params: iou loss: %s (%d), iou_norm: %2.2f, cls_norm: %2.2f, scale_x_y: %2.2f\n", iou_loss,
          l.iou_loss, l.iou_normalizer, l.cls_normalizer, l.scale_x_y);
...
dropout_layer parse_dropout(list *options, size_params params)
{
  ...
  {
    printf(" [dropout] - None of the parameters (dropblock_size_rel or dropblock_size_abs) are set, will be used: "
...
layer parse_shortcut(list *options, size_params params, network net)
{
  ...
  layer s = make_shortcut_layer(params.batch, n, layers, sizes, params.w, params.h, params.c, layers_output,
                                layers_delta, layers_output_gpu, layers_delta_gpu, weights_type, weights_normalization,
                                activation, params.train);
  ...
  for (i = 0; i < n; ++i)
  {
    ...
    if (params.w != net.layers[index].out_w || params.h != net.layers[index].out_h ||
      ...
      fprintf(stderr, " (%4d x%4d x%4d) + (%4d x%4d x%4d) \n", params.w, params.h, params.c, net.layers[index].out_w,
              net.layers[index].out_h, params.net.layers[index].out_c);
  }
...
void parse_net_options(list *options, network *net)
{
  ...
  net->init_sequential_subdivisions = net->sequential_subdivisions =
      option_find_int_quiet(options, "sequential_subdivisions", subdivs);
  ...
  *net->cur_iteration = 0;
  ...
  net->workspace_size_limit =
      (size_t)1024 * 1024 * option_find_float_quiet(options, "workspace_size_limit_MB", 1024);  // 1024 MB by default
  ...
  if (net->gpu_index >= 0)
  {
    ...
    if (compute_capability >= 700)
      ...
      net->cudnn_half = 0;
#endif  // CUDNN_HALF
    fprintf(stderr, " %d : compute_capability = %d, cudnn_half = %d, GPU: %s \n", net->gpu_index, compute_capability,
            net->cudnn_half, device_name);
  }
  else
...
network parse_network_cfg_custom(char *filename, int batch, int time_steps)
{
  ...
  while (n)
  {
    ...
    if (show_receptive_field)
    {
      ...
      // printf(" size = %d, dilation = %d, stride = %d, receptive_w = %d, receptive_w_scale = %d - ", size, dilation,
      // stride, receptive_w, receptive_w_scale);
  ...
  {
    printf("\n Warning: width=%d and height=%d in cfg-file must be divisible by 32 for default networks Yolo "
           "v1/v2/v3!!! \n\n",
           net.w, net.h);
  }
  return net;
