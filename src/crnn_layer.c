...
layer make_crnn_layer(int batch, int h, int w, int c, int hidden_filters, int output_filters, int groups, int steps,
                      int size, int stride, int dilation, int pad, ACTIVATION activation, int batch_normalize, int xnor,
                      int train)
{
  ...
  *(l.input_layer) =
      make_convolutional_layer(batch, steps, h, w, c, hidden_filters, groups, size, stride, stride, dilation, pad,
  ...
  *(l.self_layer) =
      make_convolutional_layer(batch, steps, h, w, hidden_filters, hidden_filters, groups, size, stride, stride,
  ...
  *(l.output_layer) =
      make_convolutional_layer(batch, steps, h, w, hidden_filters, output_filters, groups, size, stride, stride,
                               dilation, pad, activation, batch_normalize, 0, xnor, 0, 0, 0, 0, NULL, 0, 0, train);
...
void forward_crnn_layer_gpu(layer l, network_state state)
{
  ...
  /*
      ...
      if (!state.train && state.net.cudnn_half) {
          s.index = state.index;
          cuda_convert_f32_to_f16(input_layer.weights_gpu,
  input_layer.c*input_layer.n*input_layer.size*input_layer.size, input_layer.weights_gpu16);
          cuda_convert_f32_to_f16(self_layer.weights_gpu, self_layer.c*self_layer.n*self_layer.size*self_layer.size,
  self_layer.weights_gpu16);
          cuda_convert_f32_to_f16(output_layer.weights_gpu,
  output_layer.c*output_layer.n*output_layer.size*output_layer.size, output_layer.weights_gpu16);
      }
  #endif  //CUDNN_HALF
  */
