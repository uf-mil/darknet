...
contrastive_layer make_contrastive_layer(int batch, int w, int h, int c, int classes, int inputs, layer *yolo_layer)
{
  ...
  if (yolo_layer)
  {
    ...
    {
      printf(" Error: [contrastive] embedding_size=%d isn't equal to [yolo] embedding_size=%d. They should use the "
              ...
              for (h2 = 0; h2 < l.h; ++h2)
              {
                for (w2 = 0; w2 < l.w; ++w2)
                {
                  ...
                  if ((contrast_p_index + 1) >= contrast_p_size)
                  {
                    ...
                    // printf(" contrast_p_size = %d, z_index = %d, z_index2 = %d \n", contrast_p_size, z_index,
                    // z_index2);
                    contrast_p =
                        (contrastive_params *)xrealloc(contrast_p, contrast_p_size * sizeof(contrastive_params));
                  }
              ...
              for (n2 = 0; n2 < l.n; ++n2)
              {
                for (h2 = 0; h2 < l.h; ++h2)
                {
                  for (w2 = 0; w2 < l.w; ++w2)
                  {
                    ...
                    if (l.detection)
                    {
                      P = P_constrastive_f(z_index, z_index2, l.labels, z, l.embedding_size, l.temperature, contrast_p,
                                           contr_size);
                    }
                    else
                    {
                      P = P_constrastive(z_index, z_index2, l.labels, step, z, l.embedding_size, l.temperature,
                                         l.cos_sim, l.exp_cos_sim);
        ...
        for (w = 0; w < l.w; ++w)
        {
          ...
          if (l.detection)
          {
            ...
            grad_contrastive_loss_positive_f(z_index, l.labels, step, z, l.embedding_size, l.temperature,
                                             l.delta + delta_index, wh, contrast_p, contr_size);
          ...
          else
          {
            ...
            // positive
            grad_contrastive_loss_positive(z_index, l.labels, step, z, l.embedding_size, l.temperature, l.cos_sim,
                                           l.p_constrastive, l.delta + delta_index, wh);

            // negative
            grad_contrastive_loss_negative(z_index, l.labels, step, z, l.embedding_size, l.temperature, l.cos_sim,
                                           l.p_constrastive, l.delta + delta_index, wh);
          }
        }
      }
