...
public:
  JSON_sender(int port = 0, int _timeout = 400000) : sock(INVALID_SOCKET), timeout(_timeout)
  {
    close_all_sockets = 0;
    FD_ZERO(&master);
    if (port)
...
// Linux: fcntl() and O_NONBLOCK
  ...
  bool write(char const *outputbuf)
  {
    ...
    for (int s = 0; s <= maxfd; s++)
    {
      ...
      {
        // char head[400];
        // application/x-resource+json or application/x-collection+json -  when you are representing REST resources and
        // collections
...
int check_classes_id(detection det1, detection det2, float thresh)
{
  ...
  {
    if (det1.prob[i] > thresh && det2.prob[i] > thresh)
      return 1;
  }
  return 0;
}
                  ...
                  int deque_size, int dets_for_track, int dets_for_show)
{
  ...
  for (int old_id = 0; old_id < old_dets.size(); ++old_id)
  {
    for (int new_id = 0; new_id < new_dets_num; ++new_id)
    {
      ...
      const float sim =
          cosine_similarity(new_dets[new_id].embeddings, old_dets[old_id].embeddings, old_dets[0].embedding_size);
